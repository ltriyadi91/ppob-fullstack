...
public class AuthenticationController {
	...
	public ResponseEntity<UserRegisterDTO> registerCustomer(@RequestBody UserRegisterDTO userDto) {
		UserRegisterDTO registeredUser = authenticationService.customerSignup(userDto);
		return ResponseEntity.ok(registeredUser);
	}
	
	@PostMapping("/seller/signup")
	public ResponseEntity<UserRegisterDTO> registerSeller(@RequestBody UserRegisterDTO userDto) {
		UserRegisterDTO registeredUser = authenticationService.sellerSignup(userDto);
		return ResponseEntity.ok(registeredUser);
	}

	@PostMapping("/login")
	public ResponseEntity<LoginResponseDTO> authenticate(@RequestBody UserLoginDTO customerDto) {	       
