...
public class GeneralResponseDTO<T> {
    ...
    private void buildOk(String code, String title, String desc, T data) {
        ...
        if (code == null)
            ...
            statusCode = code;
        if (desc == null)
            statusDescription = StatusCode.SUCCESS.getDescription();
        else
    ...
    public ResponseEntity<GeneralResponseDTO<T>> okWithTitle(String title, String desc, T data) {
        buildOk(null, title, desc, data);
        return ResponseEntity.ok(this);
    }

    public ResponseEntity<GeneralResponseDTO<T>> okFull(String code, String title, String desc, T data) {
    ...
    public ResponseEntity<GeneralResponseDTO<T>> error(String code, String title, String desc, T data) {
        buildError(code, title, desc, data);
        return new ResponseEntity<>(this, HttpStatus.BAD_REQUEST);
    }

    public ResponseEntity<GeneralResponseDTO<T>> error(String code, String desc, T data, HttpStatus status) {
        buildError(code, null, desc, data);
        return new ResponseEntity<>(this, status);
    }

    public ResponseEntity<GeneralResponseDTO<T>> error(String code, String title, String desc, T data, HttpStatus status) {
        buildError(code, title, desc, data);
        return new ResponseEntity<>(this, status);
    }
