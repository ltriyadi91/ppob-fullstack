...
export default function LoginPage() {
  ...
  const { login, isLoading, error } = useAuth(); // Use the useAuth hook
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-100">
      <header className="bg-red-500 text-white p-4 flex items-center shadow-md">
        <button onClick={() => router.back()} className="mr-4 p-2 rounded-full hover:bg-red-600 transition-colors duration-200">
          <IoArrowBack size={24} />
        </button>
      ...
      <main className="flex-grow flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
          ...
          <p className="text-center text-gray-600 text-sm mt-4">
            ...
            <Link href="/register" className="text-red-500 hover:text-red-700 font-bold">
              Sign Up
            </Link>
          </p>
        </div>
